...
export const FloatingNav = ({
  ...
  const scrollToSection = (id: string, offset = -100) => {
    ...
    if (element) {
      const yOffset = offset; // Adjust the offset as needed
      const y =
  ...
  return (
    <AnimatePresence mode="wait">
      ...
      >
        {navItems?.map(
          (
            navItem: { link: string; icon?: JSX.Element; name: string },
            idx: number
          ) => (
            <Link
              key={`link=${idx}`}
              href={navItem.link}
              onClick={(e) => {
                if (navItem.link === "#contact") {
                  e.preventDefault(); // Prevent default anchor behavior
                  scrollToSection(navItem.link); // Apply custom scroll logic for "contact"
                }
              }}
              className={cn(
